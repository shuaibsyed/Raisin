 ...
 *
 * Tests calling disassemble() on a program that contains some data in its
 * data segment.
 *
...
int main(void) {
  ...
  /* load the program into memory starting at address 0, it has 15 total
     words, and its data segment is 6 words large */
  assert(load_program(memory, program, START_ADDR, PROGRAM_SIZE,
                      DATA_SEGMENT_SIZE) == 1);

  /* now disassemble, just treating the last 6 words as data instead of
     instructions */
