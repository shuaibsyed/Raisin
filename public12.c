 ...
 *
 * Tests calling disassemble() with an invalid value for start_addr.
 *
...
int main(void) {
  ...
  /* now try to disassemble starting at address 0x100000, which sadly is an
     invalid MAD Raisin address */
  assert(disassemble(memory, 0x100000, PROGRAM_SIZE, DATA_SEGMENT_SIZE) == 0);
